...
```{r Load query sheets}
sheet_AllArticles <- list(read_xlsx("Randomisation.xlsx", 
                                    sheet = "2004-2007"),
                          read_xlsx("Randomisation.xlsx", 
                                    sheet = "2008-2011"),
                          read_xlsx("Randomisation.xlsx", 
                                    sheet = "2012-2015"),
                          read_xlsx("Randomisation.xlsx", 
...
First, a table is generated presenting the number of articles using data from each country.
...
::: callout-important
## Articles may include multiple study areas in cases where distinct datasets were modelled separately
:::

```{r Study location map}
# Load country shapefile
countries_shp <- st_read("Spatial/World_Countries__Generalized_.shp", quiet = TRUE) |>
...
In this section, we gather information on practices in the modelling process to generate a summary table. This involves aspects including the median number of covariates per parameter, the proportion of studies using different model selection approaches, and the proportion of studies evaluating their models. This data is split between Bayesian and Frequentist models.
...
In this section we summarise information on the objectives of each study. Objectives were described in six non-exclusive categories.
...
We generate a plot (Figure 6 in the review) showing two elements related to objectives. First, we calculate the proportion of articles in each year-strata pursuing each objective.
...
We then render a second figure for the average number of covariates used in articles approaching each objective.
...
We combine the objective plots using the `patchwork` package.
