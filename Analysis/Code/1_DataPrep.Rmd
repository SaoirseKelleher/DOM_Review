...
survey_sheet <- spreadsheet[["Data collection"]] %>%
  ...
  mutate(MinPrimary = `Number of primary occasions`,
         MaxPrimary = `Number of primary occasions`) %>%
  map_dfr(.f = function(.x){
    if (str_detect(.x$MinPrimary, "-")){
      .x$MinPrimary <- str_split(.x$MinPrimary[[1]])
      .x$MaxPrimary <- str_split(.x$MaxPrimary[[2]])
    }
  })
  
  
  
  mutate(MinPrimary = case_when(str_detect(`Number of primary occasions`, "-") ~
                                  str_split(`Number of primary occasions`, "-")[[1]],
                                !str_detect(`Number of primary occasions`, "-") ~
                                  `Number of primary occasions`),
         MaxPrimary = case_when(str_detect(`Number of primary occasions`, "-") ~
                                  str_split(`Number of primary occasions`, "-")[[2]],
                                !str_detect(`Number of primary occasions`, "-") ~
                                  `Number of primary occasions`))
  ...
  select(`Review ID`, Title, `Study duration`, `Capture method`, 
         `Num primary` = `Number of primary occasions`,
         `Num secondary` = `Number of secondary occasions`,
         `Type secondary` = `Type of secondary occasions`,
         `Type site` = `Type of site`, `Num sites` = `Number of sites`,
         `Sampling method`)
