...
taxa_location_sheet <- spreadsheet[["Species and Location"]] %>%
  ...
  select(`Review ID`, Title, Taxa, `Taxa keywords`, `IUCN Status`, Number, 
         Threat_HabitatLoss, Threat_Exploitation, Threat_ClimateChange,
         Threat_Invasives, Threat_Disease, Threat_Disaster,
         Country, Continent, Realm, `Study Size`, `Center point`)
...
covariates_sheet <- spreadsheet[["Covariates"]] %>%
  ...
  filter(Status == "Complete") %>%
  # Select parameter prefix
  mutate(Parameter = str_split(Parameter, "_", simplify = TRUE)[,1]) %>%
  # Split out each covariate
  separate_rows(`Covariate List`, sep =  ", ") %>%
  # Convert to uppercase
  mutate(`Covariate List` = toupper(`Covariate List`)) %>%
  # Convert 'none' to NA
  separate(`Covariate List`, c("Category", "Scale", "CovObsType", "Complexity"), sep = "_") %>%
  separate(`Category`, c("CovCategory", "EnvCategory"), sep = "-") %>%
  select(`Review ID`, Title, `Total covariates`, Parameter, CovCategory, EnvCategory, Scale, 
         CovObsType, Complexity)
...
modelling_sheet <- spreadsheet[["Modelling"]] %>%
  filter(Status == "Complete") %>%
  mutate(`Prefiltering category` = tolower(`Prefiltering category`),
         `Selection category` = tolower(`Selection category`)) %>%
  select(`Review ID`, Title, Bayesian, `Prefiltering category`, `Selection category`, `Evaluation method`,
