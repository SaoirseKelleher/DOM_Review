...
focalTaxaPlot <- sheet_FocalTaxa |>
  mutate(Status = case_when(Status %in% c("CR", "EN", "NT", "Threatened_Other") ~ "Threatened",
                            Status == "NA" ~ "Multispecies",
                            Status == "Invasive" ~ "Invasive",
                            Status == "LC" ~ "Stable")) |>
  mutate(Status = fct(Status, levels = c("Multispecies", "Invasive",
                                         "Stable", "Threatened"))) |>
...
```{r Article coverage}
coveragePlot <- randomisationSheets |>
  summarise(N = n(), .by = c(Strata, Included)) |>
  pivot_wider(names_from = Included, values_from = N) |>
  mutate(HitRate = YES/(NO+YES)) |>
  mutate(EstRemaining = `NA`*HitRate) |>
  pivot_longer(cols = c(YES, EstRemaining), 
               names_to = 'Group', values_to = 'Articles') |>
  
  ggplot() +
  geom_col(aes(x = Strata, y = Articles, fill = Group), colour = "white") +
  scale_fill_manual("",
...
studyArea_Map <- sheet_StudyArea |>
  ...
  geom_sf(shape = 21,fill = "aquamarine4", colour = "white", alpha = 0.7, size = 4) +
...
siteQuantityPlot <- sheet_DataCollection |>
  ...
  geom_hline(aes(yintercept = median(`Site quantity`)), 
             colour = "goldenrod3", linetype = 2, linewidth = 2) +
  scale_y_continuous(transform = "log10",
                     limits = c(5, 10000),
...
sheet_DataCollection |>
  ...
  mutate(studyScale = case_when(studyScale > 5000 ~ 5000,
                                .default = studyScale)) |>
  mutate(outlier = case_when(studyScale >= 5000 ~ TRUE,
                             .default = FALSE)) |>
...
sheet_Objectives |>
  ...
  scale_fill_manual(values = c("plum4", "cadetblue3", "aquamarine3",
