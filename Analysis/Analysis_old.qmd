...
focalTaxaPlot <- sheet_FocalTaxa |>
  ...
  mutate(Taxa = case_when(Taxa %in% c("NA", "Fish") ~ "Other", .default = Taxa)) |>
  mutate(Taxa = fct(Taxa, levels = rev(c("Bird", "Mammal", "Herptile",
                                        "Invertebrate", "Other")))) |>
  
  ggplot() +
  geom_col(aes(x = Articles, y = Taxa, fill = Status)) +
  scale_fill_manual("", values = c("gray70", "plum4", "aquamarine4", "goldenrod2")) +
  labs(x = "Articles", y = "") +
...
sheet_FocalTaxa |>
  ...
  distinct() |>
  summarise(Articles = n(), .by = `Multispecies method`)
```
...
randomisationSheets |>
  ...
  summarise(HitRate = sum(YES)/(sum(NO)+sum(YES)),
            .by = Strata) 
  ...
  mutate(HitRate = sum(YES)/(sum(NO)+sum(YES)),
         .by = Strata) |>
  summarise(HitRate = mean(HitRate), `NA` = sum(`NA`),
            .by = Year) |>
  mutate(EstRemaining = `NA`*HitRate) |>
  summarise(`NA` = sum(`NA`), EstRemaining = sum(EstRemaining))
```
...
coveragePlot
ggsave(plot = coveragePlot, "Figures/CoveragePlot.jpeg",
       width = 8, height = 4)
```
...
sheet_DataCollection |>
  ...
  distinct() |>
  summarise(Articles = n(), .by = `Capture method`)

sheet_DataCollection |>
  select(`Review ID`, Citizen) |>
  distinct() |>
  summarise(Articles = n(), .by = Citizen)
...
studyDurationPlot <- sheet_DataCollection |>
  ...
  geom_histogram(aes(x = StudyMonths), 
                 ...
                 linewidth = 2) +
  geom_vline(aes(xintercept = median(StudyMonths)), 
...
sheet_DataCollection |>
  ...
  mutate(StudyMonths = case_when(StudyMonths == 0 ~ 1,
                                 .default = StudyMonths)) |>
  summarise(StudyMonths = mean(StudyMonths, na.rm = TRUE),
            .by = `Review ID`) |>
  pull(StudyMonths) |>
  median(na.rm = TRUE)
```
...
Combine sites and durations

```{r site and duration combination}
combinedStudyScale <- studyDurationPlot / siteQuantityPlot
...
covParamPlot
ggsave(plot = covParamPlot, filename = "Figures/CovParamPlot.jpeg",
       width = 8, height = 4)
```
...
write_csv(fullCovariateSummary, "Figures/CovariateTable.csv")
```
...
objectiveTrendPlot
ggsave(plot = objectiveTrendPlot, "Figures/ObjectiveTrend.jpeg",
       width = 8, height = 4)
```
...
ggsave(plot = objectiveCovariatePlot, "Figures/ObjectiveCovariates.jpeg",
       width = 8, height = 4)
```
...
ggsave(plot = stackedObjectivePlot, "Figures/ObjectiveFigure.jpeg",
       width = 12, height = 8)
```
